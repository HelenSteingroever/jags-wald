...
if (lambda_fixed == T){
  mf <- textConnection("model {
    ...
    for (i in 1:N) {
      x[i] ~ dwald_gamma(lambda, alpha, tau, gamma)
      #x[i] ~ dwald_trunc(lambda, alpha, tau, gamma)
    }
  }")
  
  inits1 <- list(alpha=.7, tau=.7, gamma=1)
  inits2 <- list(alpha=.9, tau=.9, gamma=1)
  ...
  inits1 <- list(lambda=.7, alpha=.7, tau=.7, gamma=.7)
  inits2 <- list(lambda=.9, alpha=.9, tau=.9, gamma=.9)
  inits3 <- list(lambda=1.1, alpha=1.1, tau=1.1, gamma=1.1)
  
  params <- c("lambda", "alpha", "tau", "gamma")
}
...
j.samples <- coda.samples(j.model, params, n.iter=1000, thin=1)
